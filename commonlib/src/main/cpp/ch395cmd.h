...
/* ********************************************************************************************************************* */
#define CMD_GET_IC_VER                          0x01                          /* 获取芯片以及固件版本号 */
/* 输出: 版本号( 位7为0, 位6为1, 位5~位0为版本号 ) */                           
/*       CH3395返回版本号的值为041H即版本号为01H */                             
                                                                                
#define CMD_SET_BAUDRATE                        0x02                          /* 串口方式: 设置串口通讯波特率(上电或者复位后的默认波特率为9600bps */
/* 输入: 3字节波特率，第字节在前 */                                             
...
#define CMD_CHECK_EXIST                         0x06                          /* 测试通讯接口以及工作状态 */
/* 输入: 任意数据 */                                                            
/* 输出: 输入数据的按位取反 */                                                  

#define CMD_GET_GLOB_INT_STATUS_ALL             0x19                          /* 获取全局中断状态，V44版本以后的程序由于增加了socket数量需要用此命令获取全部的中断 */
/*输出：全局中断状态，参考全局中断状态定义 */
                                                                                
#define CMD_SET_PHY                             0x20                          /* 设置PHY，默认为Auto，自动协商 */
/* 输入：PHY参数，参考PHY参数定义 */                                            
                                                                                
#define CMD_SET_MAC_ADDR                        0x21                          /* 设置MAC地址 必须在CMD00H_INIT_CH395之前设置完毕 */
/* 输入：6字节的MAC地址 */                                                      
                                                                                
#define CMD_SET_IP_ADDR                         0x22                          /* 设置IP地址 必须在CMD00H_INIT_CH395之前设置完毕 */
/* 输入：4字节的IP地址 */                                                       
                                                                                
#define CMD_SET_GWIP_ADDR                       0x23                          /* 设置网关IP地址 必须在CMD00H_INIT_CH395之前设置完毕 */
/* 输入：4字节的网关IP地址 */                                                   
                                                                                
#define CMD_SET_MASK_ADDR                       0x24                          /* 设置子网掩码， 必须在CMD00H_INIT_CH395之前设置完毕 */
/* 输入：4字节的子网掩码 */                                                     
                                                                                
#define CMD_SET_MAC_FILT                        0x25                          /* 设置MAC过滤 可以进行广播，多播等过滤 */
/* 输入：9字节参数，第1字节为过滤类型，参考过滤类型定义，*/
/*      第2至第5字节为HASH0，第6至第9字节为HASH1 */

#define CMD_GET_PHY_STATUS                      0x26                          /* 获取PHY当前状态，如断开连接，10/100M FULL/HALF */
/* 输出：当前PHY状态，状态定义请参考PHY参数定义 */                               
                                                                                
#define CMD_INIT_CH395                          0x27                          /* 初始化CH395 */
/* 此命令执行时间大约200MS，需要等待此命令执行成功，才可以发下一条命令 */         
                                                                                
#define CMD_GET_UNREACH_IPPORT                  0x28                          /* 获取不可达信息 */
/* 输出：8字节，第1字节为不可达类型，参考不可达类型定义 */
/*         第2字节协议不可达协议码 */
/*         第3，4字节不可达端口 */
/*         第5-8字不可达IP */

#define CMD_GET_GLOB_INT_STATUS                 0x29                          /* 获取全局中断状态，最大值为1S，不可以设置为0 */
/* 输出：全局中断状态，参考全局中断状态定义 */                                   
                                                                                
#define CMD_SET_RETRAN_COUNT                    0x2A                          /* 重试次数，仅在TCP模式下有效 */
/* 输入：重试次数 */                                                             
                                                                                
#define CMD_SET_RETRAN_PERIOD                   0x2B                          /* 重试周期，最大值为20，仅在TCP模式下有效，不可以设置为0 */
/* 输入：重试周期 */                                                             
                                                                                
#define CMD_GET_CMD_STATUS                      0x2C                          /* 获取命令执行状态 */
/* 输出：命令执行状态，参考命令执行状态定义 */                                   
                                                                                
#define CMD_GET_REMOT_IPP_SN                    0x2D                          /* 获取远端的端口以及IP地址，该命令在TCP服务器模式下使用 */
/* 输出：6字节，第1-4字节为远端的IP地址，第5-6字节为远端的端口号 */               
                                                                                
#define CMD_CLEAR_RECV_BUF_SN                   0x2E                          /* 清除接收缓冲区  */
/* 输入：第1字节为socket的索引值 */                                              
                                                                                
#define CMD_GET_SOCKET_STATUS_SN                0x2F                          /* 获取socket n状态 */
/* 输入：socket的索引值，*/                                                      
/* 输出：第1字节：socket n 打开或者关闭                                          
         第2字节：TCP状态，仅在TCP模式且第1字节为打开状态下有意义 */              
                                                                                
#define CMD_GET_INT_STATUS_SN                   0x30                          /* 获取socket n的中断状态 */
/* 输入： socket的索引值*/                                                       
/* 输出：全局中断状态，参考全局中断状态定义 */                                   
                                                                                
#define CMD_SET_IP_ADDR_SN                      0x31                          /* 设置socket n的目的IP地址 */
/* 输入：第1字节为socket的索引值，第2至5字节为IP地址 */                           
                                                                                
#define CMD_SET_DES_PORT_SN                     0x32                          /* 设置socket n的目的端口 */
/* 输入：第1字节为socket的索引值，第2至3字节为目的端口 */                         
                                                                                
#define CMD_SET_SOUR_PORT_SN                    0x33                           /* 设置socket n的源端口 */
/* 输入：第1字节为socket的索引值，第2至3字节为源端口 */                           
                                                                                
#define CMD_SET_PROTO_TYPE_SN                   0x34                          /* 设置socket n的协议类型 */
/* 输入：第1字节为socket的索引值，第2协议类型，参考协议类型定义 */                
                                                                                
#define CMD_OPEN_SOCKET_SN                      0x35                          /* 打开socket n */
/* 输入：第1字节为socket的索引值，此命令需要等待命令执行成功 */                  
                                                                                
#define CMD_TCP_LISTEN_SN                       0x36                          /* socket n监听，收到此命令，socket n进入服务器模式，仅对TCP模式有效 */
/* 输入：第1字节为socket的索引值，此命令需要等待命令执行成功 */                  
                                                                                
#define CMD_TCP_CONNECT_SN                      0x37                          /* socket n连接，收到此命令，socket n进入客户端模式，仅对TCP模式有效 */
/* 输入：第1字节为socket的索引值，此命令需要等待命令执行成功 */                  
                                                                                
#define CMD_TCP_DISNCONNECT_SN                  0x38                          /* socket n断开连接，收到此命令，socket n断开已有连接，仅对TCP模式有效 */
/* 输入：第1字节为socket的索引值，此命令需要等待命令执行成功 */                  
                                                                                
#define CMD_WRITE_SEND_BUF_SN                   0x39                          /* 向socket n缓冲区写入数据 */
/* 输入：第1字节为socket的索引值，第2至3字节为长度 */                             
                                                                                
#define CMD_GET_RECV_LEN_SN                     0x3B                          /* 获取socket n接收数据的长度 */
/* 输入：socket的索引值 */                                                        
/* 输出：2字节的接收长度 */                                                       
                                                                                
#define CMD_READ_RECV_BUF_SN                    0x3C                          /* 读取socket n接收缓冲区数据 */
/* 输入：第1字节为socket的索引值，第2至3字节为读取的长度n,低位在前 */            
/* 输出：n个数据 */                                                               
                                                                                
#define CMD_CLOSE_SOCKET_SN                     0x3D                          /* 关闭socket n */
/* 输入：socket的索引值 */                                                        
                                                                                
#define CMD_SET_IPRAW_PRO_SN                    0x3E                          /* 在IP RAW下，设置socket n的IP包协议类型 */
/* 输入：第1字节为socket的索引值,第2字节为IP RAW协议类型 */                      
                                                                                
#define CMD_PING_ENABLE                         0x3F                          /* 开启/关闭PING */
/* 输入：1字节，0为关闭PING，1为开启PING，默认开启 */                            
                                                                                
#define CMD_GET_MAC_ADDR                        0x40                          /* 获取MAC地址 */
/* 输出：6字节的MAC地址 */                                                      

#define CMD_DHCP_ENABLE                         0x41                          /* DHCP使能 */
/* 输入：1字节，1启动DHCP，0关闭DHCP */                                                                              
#define CMD_GET_DHCP_STATUS                     0x42                          /* 获取DHCP状态 */
/* 输出: 1字节状态码，0表示成功，其他值失败 */                                                                               
                                                                             
#define CMD_GET_IP_INF                         0x43                          /* IP,子网掩码，网关 */
/* 输出：20字节，分别为4字节IP，4字节网关，4字节掩码，4字节的DNS1，4字节的DNS2 */

#define CMD_PPPOE_SET_USER_NAME                 0x44                          /* 设置PPPOE用户名 */
/* 输入：N个字节，0为结束符 */                                                       
                                                                                
#define CMD_PPPOE_SET_PASSWORD                  0x45                          /* 设置密码 */
/* 输入：N个字节，0为结束符 */                                                       
                                                                                
#define CMD_PPPOE_ENABLE                        0x46                          /* PPPOE使能 */
/* 输入：1字节，1启动PPPOE，0关闭PPPOE */                                             
                                                                                
#define CMD_GET_PPPOE_STATUS                    0x47                          /* 获取pppoe状态 */
/* 输出: 1字节状态码，0表示成功，其他值失败 */                                         
                                                                                
#define CMD_SET_TCP_MSS                         0x50                          /* 设置TCP MSS */
/* 输入：TCP MSS，低位在前 */

#define CMD_SET_TTL                             0x51                          /* 设置TTL，TTL最大值为128 */
/* 输入：第1字节为socket的索引值，第2字节为TTL值，最大为128 */

#define CMD_SET_RECV_BUF                        0x52                          /* 设置SOCKET接收缓冲区 */
/* 输入：第1字节为socket的索引值，第2字节为起始块索引，第3字节为块数 */          
                                                                                
#define CMD_SET_SEND_BUF                        0x53                          /* 设置SOCKET发送缓冲区 */
/* 输入：第1字节为socket的索引值，第2字节为起始块索引，第3字节为块数 */          
                                                                                
#define CMD_SET_MAC_RECV_BUF                    0x54                          /* 设置MAC接收缓冲区 */
/* 输入：输入1字节的MAC接收缓冲区的大小，16字节为单位 */                         
                                                                                
#define CMD_SET_FUN_PARA                        0x55                          /* 设置功能参数 */
/* 输入：4字节的启动参数 */

#define CMD_SET_KEEP_LIVE_IDLE                  0x56                    /* 设置KEEPLIVE空闲 */
/*输入：4字节的保活定时器空闲时间参数，低位在前 */

#define CMD_SET_KEEP_LIVE_INTVL                 0x57                    /* 设置间隔时间 */
/*输入：4字节的保活定时器超时间隔，低位在前 */

#define CMD_SET_KEEP_LIVE_CNT                   0x58                    /* 重试次数 */
/*输入：1字节重试次数 */

#define CMD_SET_KEEP_LIVE_SN                    0X59                    /* 设置socket nkeeplive功能*/
/*输入：1个字节Socket索引，1个字节设置 */

#define CMD_EEPROM_ERASE                        0xE9                          /* 擦除EEPROM*/
                                                                                
#define CMD_EEPROM_WRITE                        0xEA                          /* 写EEPROM */
/* 输入：2字节地址，1字节长度，长度必须小于64字节 */                             
                                                                                
#define CMD_EEPROM_READ                         0xEB                          /* 读EEPROM */
/* 输入：2字节地址，1字节长度，长度必须小于64字节 */                             
                                                                                
#define CMD_READ_GPIO_REG                       0xEC                          /* 读GPIO寄存器 */
/* 输入：第1个字节为REG地址，关于地址请参考相关宏定义 */                           
                                                                              
#define CMD_WRITE_GPIO_REG                      0xED                          /* 写GPIO寄存器 */
/* 输入：第1个字节为REG地址，关于地址请参考相关宏定义 */
/*       第2个字节为数据 */
