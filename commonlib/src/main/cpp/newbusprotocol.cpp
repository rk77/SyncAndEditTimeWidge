...
static int recv_thread_stop_flag = 0;
static int send_thread_stop_flag = 0;
...
void _ttyClose(void)
{
    ...
    tidSend = -1;
    tidReceive = -1;
    while((!recv_thread_stop_flag) || (!send_thread_stop_flag))
    {
        usleep(10000);
    }
    if(ttyfd >= 0) {
...
void *frame_receive_loop(void *arg)
{
    ...
    for (;;){
        ...
        if(-1 == tidReceive) {
            LOGD("Exit frame_receive_loop\n");
            recv_thread_stop_flag = 1;
            break;
        }
