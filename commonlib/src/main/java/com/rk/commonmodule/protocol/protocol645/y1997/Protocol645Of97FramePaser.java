...
public enum Protocol645Of97FramePaser {
    PROTOCOL_645_OF_97_FRAME_PASER;
    private static final String TAG = Protocol645Of97FramePaser.class.getSimpleName();
    ...
    public synchronized Map parse(byte[] frame) {
        ...
        if (verify645Frame(frame)) {
            ...
            map.put(Protocol645Of97Constant.FRAME_OK, true);
            ...
            map.put(Protocol645Of97Constant.ADDRESS, address);
            map.put(Protocol645Of97Constant.CTRL_CODE, frame[mCtrlCodePos]);
            ...
            map.put(Protocol645Of97Constant.DATA, data);
            switch (ctrlCode) {
                case Protocol645Of97Constant.ControlCode.READ_DATA_RESPOND_OK:
                    ...
                    map.put(Protocol645Of97Constant.DATA_IDENTIFIER, dataIDString);

                    switch (dataIDString) {
                        case Protocol645Of97Constant.DataIdentifier.POSITIVE_ACTIVE_TOTAL_POWER_DI:
                            if (data.length == 6) {
                                ...
                                map.put(Protocol645Of97Constant.DataIdentifier.POSITIVE_ACTIVE__TOTAL_POWER_KEY, sb.toString());
                            }
                    ...
                    break;
                case Protocol645Of97Constant.ControlCode.READ_DATA_RESPOND_ERROR:
                    break;
                case Protocol645Of97Constant.ControlCode.READ_DATA_RESPOND_OK_CONTINUE:
                    break;
                default:
                    break;
            ...
            map.put(Protocol645Of97Constant.FRAME_OK, false);
        }
        return map;
    }
    ...
    private boolean verify645Frame(byte[] frame) {
        ...
        for (mFirstBeginPos = 0; mFirstBeginPos < frame.length; mFirstBeginPos++) {
            if (frame[mFirstBeginPos] == Protocol645Of97Constant.FRAME_BEGIN) {
                break;
            }
        }
        ...
        if (frame[mSecondBeginPos] != Protocol645Of97Constant.FRAME_BEGIN) {
            return false;
        }
        ...
        if (frame[mEndPos] != Protocol645Of97Constant.FRAME_END) {
            return false;
        }
