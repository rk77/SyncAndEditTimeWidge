...
public class Ltu645ProtocolHelper {
    ...
    public static byte[] makeDisplaceUnitAcceleratedSpeedFrame(String addr) {
        LogUtils.i("addr: " + addr);
        if (TextUtils.isEmpty(addr)) {
            return null;
        }
        String data = "00012702";
        byte ctrlCode = 0x11;
        return Protocol645FrameBaseMaker.getInstance().makeFrame(addr, ctrlCode, data);
    }

    public static String parseDisplaceUnitAcceleratedSpeedFrame(byte[] frame) {
        if (frame == null) {
            return null;
        }
        StringBuilder sb = new StringBuilder();
